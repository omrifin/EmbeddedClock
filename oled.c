...
void oledWriteCharRawPrimary( char letter ) //for large font
{
    int i;
	letter -= ' ';					// Adjust character to table that starts at 0x20
...
void oledWriteCharRawRPrimary( char letter )       //for large font
{
    int i;
	letter -= ' ';					// Adjust character to table that starts at 0x20
...
void oledWriteChar1x(char letter, unsigned char page, unsigned char column,...)
{
	...
	if(!flag)
        oledWriteCharRawR( letter );
	else 
        oledWriteCharRaw( letter ) ;
	return;
}

void oledWriteChar1xPrimary(char letter, unsigned char page, unsigned char column,...)
{
	BYTE i;
	va_list ap;
	BOOL flag = 1;
...
<<<<<<< HEAD
	...
	WriteCommand(0x10 + ((column >> 4) & 0x0F));
	if(!flag)
        oledWriteCharRawRPrimary( letter );
	else
        oledWriteCharRawPrimary( letter ) ;
=======
	WriteCommand(0x00+(column&0x0F));
	WriteCommand(0x10+((column>>4)&0x0F));
...
void oledPutStringPrimary(unsigned char *ptr, unsigned char page, unsigned char col,...)
{
	va_list ap;
	BOOL flag = 1;
    va_start(ap, col);
	flag = va_arg(ap, BOOL);
	page = page + 0xB0;
	oledWriteChar1xPrimary(*ptr, page, col, flag);
	
	if(!flag)
    {
        while(*++ptr)
            oledWriteCharRawRPrimary(*ptr);
    }
	else{
        while(*++ptr)
            oledWriteCharRawPrimary(*ptr);
    }	
=======
void oledPutString(unsigned char *ptr,unsigned char page, unsigned char col,...)
{
